...
def spark_save_to_postgress_deputesActeur(date):
    ...
    # Filtrer les données typeOrgane == "ASSEMBLEE"
    deputesActeur = deputesActeur.filter((col("typeorgane") == "ASSEMBLEE"))

    # Mettre au bon format les dates et les colonnes numériques
    deputesActeur = deputesActeur.withColumn("dateprisefonction", to_date(col("dateprisefonction"), "yyyy-MM-dd"))
    deputesActeur = deputesActeur.withColumn("placehemicycle", col("placehemicycle").cast("int"))
    deputesActeur.show()

    # Collecter les données
    data = [tuple(row) for row in deputesActeur.collect()]
    ...
    insert_query = """
        INSERT INTO depute (uid, civ, prenom, nom, typeorgane, organeref, placehemicycle, dateprisefonction)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
    """
    cur.executemany(insert_query, data)
    print("Données insérées dans la table 'depute'.")
